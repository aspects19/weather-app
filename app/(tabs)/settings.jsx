...
const settings = () => {
  ...
  const SettingCard = ({ text, IconProvider, icon, toggle }) => {
    return (
      <TouchableOpacity className="ml-4 mb-3 ">
        <BlurView
          intensity={80}
          tint="dark"
          ...
          {!toggle? 
            ...
            (toggle==="on") ? <FontAwesome6 name="toggle-on" size={24} color="white"/> :
            <Feather name='toggle-left' size={26} color="white" />
          }
  ...
  return (
    <SafeAreaView>
      <View className='h-full bg-slate-900 items-center'>
        <View className='h-40 w-full bg-slate-900 -mb-7'>
        </View>
        <View className='flex items-center content-center rounded-t-4xl bg-primary'>
          ...
          <View className='mt-3 '>
            <SettingCard IconProvider={FontAwesome} icon={"bell-o"} text={"Turn on notification"} toggle={"on"}/>
            <SettingCard IconProvider={FontAwesome6} icon={"temperature-low"} text={"Use degrees celcius"} toggle={"off"}/>
